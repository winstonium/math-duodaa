 ...
 * 如果您的模板要进行修改，请修改 templates/duodaa_temp/modules/guest/guest.html
...
if(filemtime("templates/duodaa_temp/modules/guest/guest.html") > filemtime(__file__) || (file_exists("models/modules/guest/guest.php") && filemtime("models/modules/guest/guest.php") > filemtime(__file__)) ) {
	tpl_engine("duodaa_temp","modules/guest/guest.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
?><?php foreach($guest_rs as $val){?>
	<li>
		...
		<span>
		<img style="cursor:pointer;" onclick="<?php echo $send_hi;?>(<?php echo $val["guest_user_id"];?>)" src="skin/default/jooyea/images/hi.gif" title="<?php echo $gu_langpackage->gu_greet;?>" />
		<?php if(!strpos(",,$mypals,",','.$val['guest_user_id'].',')){?>
		<img style="cursor:pointer;" onclick="<?php echo $add_friend;?>(<?php echo $val["guest_user_id"];?>)" src="skin/default/jooyea/images/add.gif" title="<?php echo $gu_langpackage->gu_add_friend;?>" />
		<?php }?>
		</span>
	</li>
<?php }?><?php } ?>