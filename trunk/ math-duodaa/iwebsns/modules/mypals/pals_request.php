 ...
 * 如果您的模板要进行修改，请修改 templates/duodaa_temp/modules/mypals/pals_request.html
...
if(filemtime("templates/duodaa_temp/modules/mypals/pals_request.html") > filemtime(__file__) || (file_exists("models/modules/mypals/pals_request.php") && filemtime("models/modules/mypals/pals_request.php") > filemtime(__file__)) ) {
	tpl_engine("duodaa_temp","modules/mypals/pals_request.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php 
...
<?php foreach($request_rs as $rs){?>
    ...
    <td width="20">
       <a href='do.php?act=del_req&req_id=<?php echo $rs['id'];?>&req_uid=<?php echo $rs['req_id'];?>' onclick='return confirm("<?php echo $mp_langpackage->mp_con_del_req;?>")';><img title="<?php echo $mp_langpackage->mp_del_req;?>" src="skin/default/jooyea/images/del.png" /></a>
    </td>
	</tr>
<?php }?>
</table>
